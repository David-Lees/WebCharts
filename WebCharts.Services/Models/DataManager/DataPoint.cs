...
namespace WebCharts.Services
{
    ...
    public class CustomProperties
    {
        ...
        internal virtual DataPointCustomProperties DataPointCustomProperties { get; set; }
        ...
        internal virtual string GetUserDefinedCustomProperties(bool userDefined)
        {
            ...
            StringBuilder userDefinedCustomAttribute = new();
            ...
            if (customAttribute.Length > 0)
            {
                ...
                foreach (string nameValue in nameValueStrings)
                {
                    ...
                    if (userDefinedAttribute == userDefined)
                    {
                        ...
                        {
                            userDefinedCustomAttribute.Append(", ");
                        }

                        string val = values[1].Replace("\\x45", ",");
                        val = val.Replace("\\x46", "=");
                        userDefinedCustomAttribute.Append(values[0] + "=" + val);
                    }
                }
            }

            return userDefinedCustomAttribute.ToString();
        }
    ...
    public class DataPoint : DataPointCustomProperties
    {
        ...
        public void SetValueXY(object xValue, params object[] yValue)
        {
            ...
            if (series != null && xValue is DateTime time1)
            {
                    ...
                    else
                    {
                        if (series.YValueType == ChartValueType.Date)
                        {
                            ...
                            if (yValue[i] is DateTime time)
                                yDate = time;
                            else if (yValue[i] is double dd)
                                yDate = DateTime.FromOADate(dd);
                            else
                    ...
                    if (closingBracketIndex >= keyEndIndex)
                    {
                        ...
                        else
                        {
                            ...
                            if (properties is DataPoint dataPoint && dataPoint.series != null
                                && dataPoint.series.IsCustomPropertySet(attributeName))
                            {
        ...
        private static double ConvertValue(object value)
        {
            ...
            else if (value is UInt32 ui32)
            {
                return ui32;
            }
            else if (value is Int64 asInt64)
            {
                return asInt64;
            }
            else if (value is UInt64 asUInt64)
            {
                return asUInt64;
            }
        ...
        private static string RemoveOneKeyword(string strOriginal, string keyword, string strToReplace)
        {
            string result = strOriginal;
            int keyIndex;
            ...
            ref string[] otherValueFormat)
        {
            if (otherFields != null && otherFields.Length > 0)
            {
                ...
                for (int index = 0; index < otherAttributeNames.Length; index++)
                {
                    ...
                    {
                        otherValueFormat[index] = otherFieldNames[index][(bracketIndex + 1)..].Trim('{', '}');
            ...
            set
            {
                ...
                if (value.Length > 0)
                {
                    ...
                    foreach (string nameValue in nameValueStrings)
                    {
                        ...
                        {
                            if (existingAttributeName is string existingAttributeNameStr && string.Compare(existingAttributeNameStr, values[0], StringComparison.OrdinalIgnoreCase) == 0)
                            {
