...
namespace WebCharts.Services.Models.ChartTypes
{
    ...
    internal class KagiChart : StepLineChart
	{
		...
		internal	SKColor	kagiUpColor = SKColor.Empty;
			...
			float tension)
		{
			...
			if(pointIndex >= 2)
			{
				...
				if(direction != currentKagiDirection)
				{
					...
					SKPoint	prevPoint = points[pointIndex - 2];
		...
		override protected SKPoint[] GetPointsPosition(ChartGraphics graph, Series series, bool indexedSeries)
		{
			...
			bool clippedSegment)
		{
			...
			SKPath[] resultPathLine = new SKPath[3];
