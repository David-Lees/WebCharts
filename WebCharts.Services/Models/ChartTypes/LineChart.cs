...
namespace WebCharts.Services
{
    ...
    internal class SplineChart : LineChart
    {
        ...
        /// <summary>
        /// Fills a SKPoint array of data points positions.
        /// </summary>
        ...
        override protected SKPoint[] GetPointsPosition(
            ChartGraphics graph,
            Series series,
            bool indexedSeries)
        {
    ...
    internal class LineChart : PointChart
    {
        ...
        private SKPaint _linePen = new() { Color = SKColors.Black, Style = SKPaintStyle.Stroke };
            ...
            Series seriesToDraw)
        {
            ...
            foreach (Series ser in common.DataManager.Series)
            {
                ...
                SKPoint[] dataPointPos = null;
                if (lineTension == 0 && !common.ProcessModeRegions)
                {
                    dataPointPos = new SKPoint[ser.Points.Count];
                }
                else
                {
                ...
                //************************************************************
                    ...
                    ser.Points.Count == 1 && !_processBaseChart && (alwaysDrawMarkers ||
        ...
        private static void DrawTruncatedLine(ChartGraphics graph, SKPaint pen, SKPoint pt1, SKPoint pt2)
        {
            ...
            SKRect rect = new(0, 0, graph.Common.ChartPicture.Width, graph.Common.ChartPicture.Height);
            SKPoint adjustedPoint1;
            SKPoint adjustedPoint2;
            if (topBottomLine)
            {
            ...
            float tension)
        {
            ...
            {
                SKColor color = (useBorderColor) ? point.BorderColor : point.Color;
                ...
                if (!disableShadow && series.ShadowOffset != 0 && series.ShadowColor != SKColors.Empty && color != SKColor.Empty && color != SKColors.Transparent && pointBorderWidth > 0 && dashStyle != ChartDashStyle.NotSet)
                {
                    ...
                    // Translate curve
                    SKMatrix transform = graph.Transform;
                ...
                if (color != SKColor.Empty && pointBorderWidth > 0 && dashStyle != ChartDashStyle.NotSet)
                {
                    if (_linePen.Color != color)
                    {
                        _linePen.Color = color;
                    }
                    if (_linePen.StrokeWidth != pointBorderWidth)
                    {
                        _linePen.StrokeWidth = pointBorderWidth;
                    }
                    if (_linePen.PathEffect != ChartGraphics.GetPenStyle(dashStyle, pointBorderWidth))
                    {
                        _linePen.PathEffect = ChartGraphics.GetPenStyle(dashStyle, pointBorderWidth);
                    }
            ...
            if (common.ProcessModeRegions)
            {
                ...
                using (SKPath path = new())
                {
                    ...
                    if (lineTension == 0)
                    {
                        ...
                        if (pointIndex > 0)
                        {
                            SKPoint first = points[pointIndex - 1];
                            SKPoint second = points[pointIndex];
                        ...
                        {
                            SKPoint first = points[pointIndex];
                            SKPoint second = points[pointIndex + 1];
        ...
        /// <param name="indexedSeries">Indicate that point index should be used as X value.</param>
        /// <returns>Array of data points position.</returns>
        virtual protected SKPoint[] GetPointsPosition(ChartGraphics graph, Series series, bool indexedSeries)
        {
            SKPoint[] pointPos = new SKPoint[series.Points.Count];
            int index = 0;
            ...
            Series seriesToDraw)
        {
            ...
            graph.frontLinePoint2 = SKPoint.Empty;
            ...
            for (int pointsLoop = 0; pointsLoop < allPointsLoopsNumber; pointsLoop++)
            {
                ...
                foreach (object obj in dataPointDrawingOrder)
                {
                    ...
                    if (pointEx.index > 1)
                    {
                        ...
                        if (pointAttr.dataPoint.Color != SKColor.Empty)
                        {
        ...
        protected virtual SKPath Draw3DSurface(
            ChartArea area,
            ChartGraphics graph,
            Matrix3D matrix,
        ...
        protected static int GetCenterPointIndex(ArrayList points)
        {
            ...
            float bottomDarkening)
        {
                ...
                (decimal)secondPoint.yPosition > plotAreaPositionBottom)
            {
                ...
                {
                    _ = Draw3DSurface(firstPoint, secondPoint, reversed,
                ...
                for (int segmentIndex = 0; segmentIndex < 3; segmentIndex++)
                {
                    ...
                    if (segmentIndex == 1 && intersectionPoint2 != null && segmentNumber == 3)
                    {
                        ...
                        segmentPath = Draw3DSurface(intersectionPoint, intersectionPoint2, reversed,
                            ...
                            new SKPoint((float)intersectionPoint.xPosition, float.NaN),
                            new SKPoint((float)intersectionPoint2.xPosition, float.NaN),
                            false,
                            false, true);
                    }
        ...
        protected bool ClipBottomPoints(
            ...
            ref DataPoint3D firstPoint,
            ref DataPoint3D secondPoint,
            ref SKPoint thirdPoint,
            ref SKPoint fourthPoint,
            bool reversed,
            ChartArea area,
            ChartGraphics graph,
            Matrix3D matrix,
                ...
                (decimal)fourthPoint.Y > plotAreaPositionBottom)
            {
                ...
                if (surfaceCompletlyOutside)
                {
                    resultPath = Draw3DSurface(firstPoint, secondPoint, reversed,
                        ...
                        tension, operationType, surfaceSegmentType,
                        topDarkening, 0.5f,
                        new SKPoint(thirdPoint.X, thirdPoint.Y),
                        new SKPoint(fourthPoint.X, fourthPoint.Y),
                        outsideTop,
                        false, false);

                    // Restore previous x\y positions
                    thirdPoint = new SKPoint(prevThirdPoint.X, prevThirdPoint.Y);
                    fourthPoint = new SKPoint(prevFourthPoint.X, prevFourthPoint.Y);
                ...
                for (int segmentIndex = 0; segmentIndex < 3; segmentIndex++)
                {
                        ...
                        segmentIndex == 2 && reversed)
                    {
                        ...
                        segmentPath = Draw3DSurface(firstPoint, intersectionPoint2, reversed,
                            ...
                            new SKPoint(float.NaN, thirdPoint.Y),
                            new SKPoint((float)intersectionPoint2.xPosition, (!firstSegmentVisible || segmentNumber == 3) ? thirdPoint.Y : fourthPoint.Y),
                    ...
                    if (segmentIndex == 1 && intersectionPoint2 != null && segmentNumber == 3)
                    {
                        ...
                        segmentPath = Draw3DSurface(intersectionPoint, intersectionPoint2, reversed,
                            ...
                            tension, operationType, LineSegmentType.Middle,
                            topDarkening, bottomDarkening,
                            new SKPoint((float)intersectionPoint.xPosition, thirdPoint.Y),
                            new SKPoint((float)intersectionPoint2.xPosition, fourthPoint.Y),
                            false,
                            false, false);
                        ...
                        segmentIndex == 0 && reversed)
                    {
                        ...
                        segmentPath = Draw3DSurface(intersectionPoint, secondPoint, reversed,
                            ...
                            (surfaceSegmentType == LineSegmentType.Middle) ? LineSegmentType.Middle : LineSegmentType.Last,
                            topDarkening, bottomDarken,
                            new SKPoint((float)intersectionPoint.xPosition, thirdPointNewY),
                            new SKPoint(float.NaN, fourthPoint.Y),
