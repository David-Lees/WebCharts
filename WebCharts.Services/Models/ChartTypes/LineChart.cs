...
namespace WebCharts.Services.Models.ChartTypes
{
	...
	internal class LineChart : PointChart
	{
		...
		// VSTS: 9698 - issue: the line start from X = 0 when GDI overflows (before we expected exception)
		private bool IsLinePointsOverflow(SKPoint point)
		{
		...
		private void DrawTruncatedLine(ChartGraphics graph, Pen pen, SKPoint pt1, SKPoint pt2)
		{
			SKPoint adjustedPoint1 = SKPoint.Empty;
			SKPoint adjustedPoint2 = SKPoint.Empty;
			...
			SKRect rect = new SKRect(0, 0, graph.Common.ChartPicture.Width, graph.Common.ChartPicture.Height);
			if (topBottomLine)
			{
