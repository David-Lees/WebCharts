    ...
    internal class PointAndFigureChart : RangeColumnChart
	{
			...
			double maxPrice)
		{
            ...
            if (series.IsCustomPropertySet(CustomPropertyName.BoxSize))
            {
                ...
                if (!parseSucceed)
                {
                    throw (new InvalidOperationException(SR.ExceptionRenkoBoxSKSizeormatInvalid));
                }
            }
				...
				if(series.Name.StartsWith("POINTANDFIGURE_ORIGINAL_DATA_", StringComparison.Ordinal))
				{
						...
						pointAndFigureSeries.ChartArea.Length > 0)
					{
						...
						if(!chartArea.Area3DStyle.Enable3D)
						{
							...
							SKSize markSize = chart.chartPicture.ChartGraph.GetAbsoluteSize(
								new SKSize((float)boxXSize, (float)boxYSize));
				...
				if(point.IsCustomPropertySet("PriceUpPoint"))
				{
					...
					if(ser.ShadowOffset != 0)
					{
						graph.DrawLineAbs( 
							...
							new SKPoint(shadowPosition.Left, shadowPosition.Top),
							new SKPoint(shadowPosition.Right, shadowPosition.Bottom));
						graph.DrawLineAbs( 
							ser.ShadowColor, 
							point.BorderWidth, 
							ChartDashStyle.Solid, 
							new SKPoint(shadowPosition.Left, shadowPosition.Bottom),
							new SKPoint(shadowPosition.Right, shadowPosition.Top));
					}
					...
					graph.DrawLineAbs( 
						...
						point.BorderWidth, 
						ChartDashStyle.Solid, 
						new SKPoint(position.Left, position.Top),
						new SKPoint(position.Right, position.Bottom));
					graph.DrawLineAbs( 
						point.Color, 
						point.BorderWidth, 
						ChartDashStyle.Solid, 
						new SKPoint(position.Left, position.Bottom),
						new SKPoint(position.Right, position.Top));
				}
				else
				{
