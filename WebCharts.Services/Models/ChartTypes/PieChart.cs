	...
	internal class PieChart : IChartType
	{
		...
		private void ProcessChartType( bool selection, ChartGraphics graph, CommonElements common, ChartArea area, bool shadow, LabelsMode labels )
		{
			...
			{	
				...
				if( area.InnerPlotPosition.Auto )
				{
					rectangle = new SKRect( area.Position.ToSKRect().X, area.Position.ToSKRect().Y, area.Position.ToSKRect().Width, area.Position.ToSKRect().Height );
				}
				else
				{
					rectangle = new SKRect( area.PlotAreaPosition.ToSKRect().X, area.PlotAreaPosition.ToSKRect().Y, area.PlotAreaPosition.ToSKRect().Width, area.PlotAreaPosition.ToSKRect().Height );
				}
				...
				// Center of the pie
				SKPoint middlePoint = new SKPoint( rectangle.X + rectangle.Width / 2, rectangle.Y + rectangle.Height / 2 );
    ...
    (point.IsCustomPropertySet(CustomPropertyName.PieLineColor)) ? point[CustomPropertyName.PieLineColor] : dataSeries[typeSeries[0]][CustomPropertyName.PieLineColor]);
				...
				if( common.ProcessModePaint )
				{
					...
					if( shadow )
					{
						...
						// Shadow Rectangle
						SKRect shadowRect = new SKRect( rectangle.X, rectangle.Y, rectangle.Width, rectangle.Height );
			...
			{
				this._labelsOverlap = PrepareLabels( area.Position.ToSKRect() );
			}
		...
		public void DrawLabels( ChartGraphics graph, SKPoint middlePoint, SKSize relativeSize, float startAngle, float sweepAngle, DataPoint point, float doughnutRadius, bool exploded, ChartArea area, bool overlapTest, int pointIndex, Color pieLineColor )
		{
			...
			else if( style == PieLabelStyle.Outside )
			{
                ...
                using (StringFormat format = new StringFormat())
                {
                    ...
                    if (!overlapTest)
                    {
                        if (this._labelsOverlap)
                        {
                            float calculatedY3 = (((SKRect)this._labelsRectangles[pointIndex]).Top + ((SKRect)this._labelsRectangles[pointIndex]).Bottom) / 2f;
                    ...
                    if (!overlapTest)
                    {
                        SKRect rect = new SKRect(labelRect.Location, labelRect.Size);
                        if (this._labelsOverlap)
                        {
                            // Draw label from collection if original labels overlap.
                            rect = (SKRect)this._labelsRectangles[pointIndex];
                        ...
                        SKRect labelBackPosition = new SKRect(
                            rect.X,
		...
		private SKRect GetLabelRect( SKPoint labelPosition, ChartArea area, string text, StringFormat format, ChartGraphics graph, DataPoint point, bool leftOrientation )
		{
			SKRect labelRect = SKRect.Empty;
			if( leftOrientation )
			{
		...
		/// <param name="graph">Chart graphics object</param>
		/// <param name="pointIndex">Data point index</param>
		private void Map( CommonElements common, DataPoint point, float startAngle, float sweepAngle, SKRect rectangle, bool doughnut, float doughnutRadius, ChartGraphics graph, int pointIndex )
		{
		...
		private bool PrepareLabels( SKRect area )
		{
			...
			foreach( SKRect rect in this._labelsRectangles )
			{
			...
			float pieWidth )
		{
			...
			SKRect plotingRectangle = new SKRect( area.Position.ToSKRect().X + 1, area.Position.ToSKRect().Y + 1, area.Position.ToSKRect().Width-2, area.Position.ToSKRect().Height-2 );
			...
			for( int turn = 0; turn < 5; turn++ )
			{
				...
				foreach( DataPoint point in points )
				{
					...
					SKRect	rectangle;
					if( area.InnerPlotPosition.Auto )
						rectangle = new SKRect( plotingRectangle.X, plotingRectangle.Y, plotingRectangle.Width, plotingRectangle.Height );
					else
						rectangle = new SKRect( area.PlotAreaPosition.ToSKRect().X, area.PlotAreaPosition.ToSKRect().Y, area.PlotAreaPosition.ToSKRect().Width, area.PlotAreaPosition.ToSKRect().Height );
			...
			)
		{
            ...
            using (SKPath path = new SKPath())
            {
                ...
                {
                    brush = graph.GetGradientBrush(graph.GetAbsoluteRectangle(area.Position.ToSKRect()), Color.FromArgb(brushWithoutLight.Color.A, 0, 0, 0), brushWithoutLight.Color, GradientStyle.VerticalCenter);
                }
			...
			)
		{
			...
			SKSize fontSize = new SKSize(1.4F * series.Font.Size,1.4F * series.Font.Size);
			...
			int pointIndex)
		{
            ...
            using (StringFormat format = new StringFormat())
            {
                ...
                if (midAngle >= -90 && midAngle < 90 || midAngle >= 270 && midAngle < 450)
                {
                    ...
                    float labelVertSize = graph.GetAbsoluteSize(new SKSize(0f, this.labelColumnRight.columnHeight)).Height;
                ...
                else
                {
                    ...
                    float labelVertSize = graph.GetAbsoluteSize(new SKSize(0f, this.labelColumnLeft.columnHeight)).Height;
                    labelPoint = graph.GetAbsolutePoint(columnLabel.GetLabelPosition(point));
