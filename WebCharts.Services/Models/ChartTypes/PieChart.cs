...
namespace WebCharts.Services
{
    ...
    internal class PieChart : IChartType
    {
            ...
            if (threshold > 0.0)
            {
                ...
                if (collectedCount > 1 && collectedPoint != null)
                {
                    ...
                    if (series.IsCustomPropertySet(CustomPropertyName.CollectedColor))
                    {
                        try
                        {
                            collectedPoint.Color = SKColor.Parse(series[CustomPropertyName.CollectedColor]);
                        }
                        catch
                        {
        ...
        private void ProcessChartType(bool selection, ChartGraphics graph, CommonElements common, ChartArea area, bool shadow, LabelsMode labels)
        {
            ...
            foreach (DataPoint point in dataSeries[typeSeries[0]].Points)
            {
                ...
                if (area.InnerPlotPosition.Auto)
                {
                    rectangle = new SKRect(area.Position.ToSKRect().Left, area.Position.ToSKRect().Top, area.Position.ToSKRect().Width, area.Position.ToSKRect().Height);
                }
                else
                {
                    rectangle = new SKRect(area.PlotAreaPosition.ToSKRect().Left, area.PlotAreaPosition.ToSKRect().Top, area.PlotAreaPosition.ToSKRect().Width, area.PlotAreaPosition.ToSKRect().Height);
                }
                ...
                // Find smallest edge
                SKSize absoluteSize = graph.GetAbsoluteSize(new SKSize(rectangle.Width, rectangle.Height));
                ...
                if (_sizeCorrection != 1)
                {
                    ...
                    rectangle.Left += rectangle.Width * (1 - _sizeCorrection) / 2;
                    rectangle.Top += rectangle.Height * (1 - _sizeCorrection) / 2;
                    rectangle.Right = rectangle.Left + w * _sizeCorrection;
                    rectangle.Bottom = rectangle.Top + h * _sizeCorrection;
                    ...
                    if (area.InnerPlotPosition.Auto)
                    {
                        ...
                        rect.Size = new(w / area.Position.Width * 100f, h / area.Position.Height * 100f);
                ...
                SKColor pieLineColor = SKColor.Empty;
    ...
    (point.IsCustomPropertySet(CustomPropertyName.PieLineColor)) ? point[CustomPropertyName.PieLineColor] : dataSeries[typeSeries[0]][CustomPropertyName.PieLineColor]);
                ...
                if (common.ProcessModePaint)
                {
                    ...
                    if (shadow)
                    {
                        double offset = graph.GetRelativeSize(new SKSize(point.series.ShadowOffset, point.series.ShadowOffset)).Width;
                        ...
                        // Shadow Rectangle
                        SKRect shadowRect = new(rectangle.Left, rectangle.Top, rectangle.Width, rectangle.Height);
                        shadowRect.Offset((float)offset, (float)offset);

                        // Change shadow color
                        SKColor shcolor;
                        SKColor shGradientColor;
                        SKColor shBorderColor;

                        // Solid color
                        if (point.Color.Alpha != 255)
                            shcolor = Color.FromArgb(point.Color.Alpha / 2, point.series.ShadowColor);
                        else
                            shcolor = point.series.ShadowColor;

                        // Gradient Color
                        if (point.BackSecondaryColor != SKColor.Empty)
                        {
                            if (point.BackSecondaryColor.Alpha != 255)
                                shGradientColor = Color.FromArgb(point.BackSecondaryColor.Alpha / 2, point.series.ShadowColor);
                            else
                                shGradientColor = point.series.ShadowColor;
                        }
                        else
                            shGradientColor = SKColor.Empty;

                        // Border color
                        if (point.BorderColor != SKColor.Empty)
                        {
                            if (point.BorderColor.Alpha != 255)
                                shBorderColor = Color.FromArgb(point.BorderColor.Alpha / 2, point.series.ShadowColor);
                            else
                                shBorderColor = point.series.ShadowColor;
                        }
                        else
                            shBorderColor = SKColor.Empty;
            ...
            {
                _labelsOverlap = PrepareLabels(area.Position.ToSKRect());
            }
        ...
        public void DrawLabels(ChartGraphics graph, SKPoint middlePoint, SKSize relativeSize, float startAngle, float sweepAngle, DataPoint point, float doughnutRadius, bool exploded, ChartArea area, bool overlapTest, int pointIndex, SKColor pieLineColor)
        {
            ...
            else if (style == PieLabelStyle.Outside)
            {
                ...
                if (!overlapTest)
                {
                    graph.DrawLineRel(pieLineColor, point.BorderWidth, ChartDashStyle.Solid, new SKPoint(x1, y1), new SKPoint(x2, y2));
                }
            ...
            if (!added)
            {
                InsertOverlapLabel(SKRect.Empty);
            }
        ...
        private static SKRect GetLabelRect(SKPoint labelPosition, ChartArea area, string text, StringFormat format, ChartGraphics graph, DataPoint point, bool leftOrientation)
        {
            SKRect labelRect = SKRect.Empty;
            if (leftOrientation)
            {
                labelRect.Left = area.Position.X;
                labelRect.Top = area.Position.Y;
                labelRect.Right = labelRect.Left + labelPosition.X - area.Position.X;
                labelRect.Bottom = labelRect.Top + area.Position.Height;
            }
            else
            {
                labelRect.Left = labelPosition.X;
                labelRect.Top = area.Position.Y;
                labelRect.Right = labelRect.Left + area.Position.Right - labelPosition.X;
                labelRect.Bottom = labelRect.Top + area.Position.Height;
            }

            // Find bounding rectangle of the text
            SKSize size = graph.MeasureStringRel(text.Replace("\\n", "\n"), point.Font, labelRect.Size, format);
            labelRect.Top = labelPosition.Y - size.Height / 2 * 1.8f;
            labelRect.Bottom = labelRect.Top + size.Height * 1.8f;
        ...
        /// <param name="exploded">The pie slice is exploded</param>
        /// <param name="area">Chart area</param>
        public bool EstimateLabels(ChartGraphics graph, SKPoint middlePoint, SKSize relativeSize, float startAngle, float sweepAngle, DataPoint point, bool exploded, ChartArea area)
        {
            float labelsHorizontalLineSize = 1; // Horizontal line size for outside labels
            float labelsRadialLineSize = 1; // Radial line size for outside labels
            float shift;
            ...
            if (style == PieLabelStyle.Outside)
            {
                ...
                else
                {
                    if (midAngle > 90 && midAngle < 270)
                    {
                        if (x3 - size.Width < area.PlotAreaPosition.ToSKRect().Left)
                            _labelsFit = false;
                    }
                    else
                    {
                        if (x3 + size.Width > area.PlotAreaPosition.ToSKRect().Right)
                            _labelsFit = false;
                    }

                    if (midAngle > 180 && midAngle < 360)
                    {
                        if (y3 - size.Height / 2 < area.PlotAreaPosition.ToSKRect().Top)
                            _labelsFit = false;
                    }
                    else
                    {
                        if (y3 + size.Height / 2 > area.PlotAreaPosition.ToSKRect().Bottom)
                            _labelsFit = false;
                    }
                }
            }
        ...
        private static void Map(CommonElements common, DataPoint point, float startAngle, float sweepAngle, SKRect rectangle, bool doughnut, float doughnutRadius, ChartGraphics graph, int pointIndex)
        {
            ...
            doughnutRect.Left = rectangle.Left + rectangle.Width * (1 - (100 - doughnutRadius) / 100) / 2;
            doughnutRect.Top = rectangle.Top + rectangle.Height * (1 - (100 - doughnutRadius) / 100) / 2;
            doughnutRect.Size = new(rectangle.Width * (100 - doughnutRadius) / 100, rectangle.Height * (100 - doughnutRadius) / 100);
            ...
            path.AddPie(rectangle.Left, rectangle.Top, rectangle.Width, rectangle.Height, startAngle, sweepAngle);
            ...
            if (doughnut)
            {
                ...
                path.AddPie(doughnutRect.Left, doughnutRect.Top, doughnutRect.Width, doughnutRect.Width, startAngle, sweepAngle);
            }
            ...
            SKPoint[] pointNew = new SKPoint[path.PointCount];
            for (int i = 0; i < path.PointCount; i++)
            {
                pointNew[i] = graph.GetRelativePoint(path.Points[i]);
            }
        ...
        private bool PrepareLabels(SKRect area)
        {
            // Initialization of local variables
            float splitPoint = area.Left + area.Width / 2f;
            int numberOfLeft = 0;
            int numberOfRight = 0;

            // Find the number of left and right labels.
            foreach (SKRect rect in _labelsRectangles)
            {
                if (rect.Left < splitPoint)
                {
                    numberOfLeft++;
                }
                else
            ...
            if (numberOfLeft > 0)
            {
                ...
                {
                    SKRect rect = (SKRect)_labelsRectangles[index];
                    if (rect.Left < splitPoint)
                    {
                ...
                if (ArrangeOverlappingIntervals(startPoints, endPoints, area.Top, area.Bottom))
                {
                    ...
                    for (int index = 0; index < _labelsRectangles.Count; index++)
                    {
                        ...
                        if (rect.Left < splitPoint)
                        {
                            rect.Top = (float)startPoints[splitIndex];
                            rect.Bottom = rect.Top + (float)(endPoints[splitIndex] - rect.Top);
            ...
            if (numberOfRight > 0)
            {
                ...
                if (ArrangeOverlappingIntervals(startPoints, endPoints, area.Top, area.Bottom))
                {
                    ...
                    for (int index = 0; index < _labelsRectangles.Count; index++)
                    {
                        ...
                        {
                            rect.Top = (float)startPoints[splitIndex];
                            rect.Bottom = (float)(endPoints[splitIndex]);
                            _labelsRectangles[positionIndex[splitIndex]] = rect;
                            splitIndex++;
                        }
                    }
        ...
        private void InsertOverlapLabel(SKRect labelRect)
        {
            ...
            float pieWidth)
        {
            ...
            SKRect plotingRectangle = new(area.Position.ToSKRect().Left + 1, area.Position.ToSKRect().Top + 1, area.Position.ToSKRect().Width - 2, area.Position.ToSKRect().Height - 2);
            ...
            for (int turn = 0; turn < 5; turn++)
            {
                ...
                foreach (DataPoint point in points)
                {
                    ...
                    point.positionRel = SKPoint.Empty;
                    ...
                    SKRect rectangle;
                    if (area.InnerPlotPosition.Auto)
                        rectangle = new SKRect(plotingRectangle.Left, plotingRectangle.Top, plotingRectangle.Width, plotingRectangle.Height);
                    else
                        rectangle = new SKRect(area.PlotAreaPosition.ToSKRect().Left, area.PlotAreaPosition.ToSKRect().Top, area.PlotAreaPosition.ToSKRect().Width, area.PlotAreaPosition.ToSKRect().Height);
                    ...
                    if (isExploded)
                    {
                        ...
                        rectangle.Left += rectangle.Width * (1 - sizeCorrection) / 2;
                        rectangle.Top += rectangle.Height * (1 - sizeCorrection) / 2;
                        rectangle.Size = new(rectangle.Width * sizeCorrection, rectangle.Height * sizeCorrection);
                    }
                    ...
                    if (turn == 1 && GetLabelStyle(point) == PieLabelStyle.Outside)
                    {
                        FillPieLabelOutside(graph, area, rectangle, pieWidth, point, startAngle, sweepAngle, pointIndx, doughnutRadius, exploded);
                    }
                    if (turn == 2 && GetLabelStyle(point) == PieLabelStyle.Outside && pointIndx == 0)
                    {
            ...
            )
        {
            ...
            SKColor penColor;
            if (point.BorderColor == SKColor.Empty && area.Area3DStyle.LightStyle == LightStyle.None)
            {
                penColor = ChartGraphics.GetGradientColor(point.Color, SKColors.Black, 0.5);
            }
            else if (point.BorderColor == SKColor.Empty)
            {
            ...
            SKPaint pen = new() { Color = penColor, StrokeWidth = point.BorderWidth, Style = SKPaintStyle.Stroke };
            pen.PathEffect = ChartGraphics.GetPenStyle(point.BorderDashStyle, point.BorderWidth);

            // Pen for back side slice.
            SKPaint backSlicePen;
            if (point.BorderColor == SKColor.Empty)
            {
                backSlicePen = new() { Color = point.Color };
            }
            else
            {
                backSlicePen = pen;
            ...
            // Set Border Width
            SKPoint[] points = GetPiePoints(graph, area, pieWidth, rectangle, startAngle, sweepAngle, true, doughnutRadius, exploded);
            ...
            else if (turn == 3)
            {
                ...
                {
                    // Check if special color properties are set
                    SKColor pieLineColor = pen.Color;
                    if (point.IsCustomPropertySet(CustomPropertyName.PieLineColor) || (point.series != null && point.series.IsCustomPropertySet(CustomPropertyName.PieLineColor)))
                    {
                        ...
                        try
                        {
                            ...
                            {
                                pieLineColor = SKColor.Parse(point[CustomPropertyName.PieLineColor]);
                            }
                            else if (point.series != null && point.series.IsCustomPropertySet(CustomPropertyName.PieLineColor))
                            {
                                pieLineColor = SKColor.Parse(point.series[CustomPropertyName.PieLineColor]);
                            }
                        }
                        catch (ArgumentException)
                        {
                    ...
                    // Draw labels
                    using SKPaint labelPen = new() { Color = pieLineColor, StrokeWidth = pen.StrokeWidth, Style = SKPaintStyle.Stroke };
            ...
            )
        {
            ...
            if (Doughnut)
            {
                ...
                result = new SKPoint[29];
            }
            else
            {
                points = new Point3D[17];
                result = new SKPoint[17];
            }

            // Angle 180 Top point on the arc
            points[(int)PiePoints.Top180] = new Point3D(
                rectangle.Left + (float)Math.Cos(180 * Math.PI / 180) * rectangle.Width / 2F + rectangle.Width / 2F,
                rectangle.Top + (float)Math.Sin(180 * Math.PI / 180) * rectangle.Height / 2F + rectangle.Height / 2F,
                pieWidth);
            ...
            points[(int)PiePoints.Top0] = new Point3D(
                rectangle.Left + (float)Math.Cos(0 * Math.PI / 180) * rectangle.Width / 2F + rectangle.Width / 2F,
                rectangle.Top + (float)Math.Sin(0 * Math.PI / 180) * rectangle.Height / 2F + rectangle.Height / 2F,
                pieWidth);
            ...
            rectangle.Left + (float)Math.Cos(startAngle * Math.PI / 180) * rectangle.Width / 2F + rectangle.Width / 2F,
            rectangle.Top + (float)Math.Sin(startAngle * Math.PI / 180) * rectangle.Height / 2F + rectangle.Height / 2F,
            pieWidth);

            // Top End Angle point on the arc
            points[(int)PiePoints.TopEnd] = new Point3D(
            rectangle.Left + (float)Math.Cos((startAngle + sweepAngle) * Math.PI / 180) * rectangle.Width / 2F + rectangle.Width / 2F,
            rectangle.Top + (float)Math.Sin((startAngle + sweepAngle) * Math.PI / 180) * rectangle.Height / 2F + rectangle.Height / 2F,
            pieWidth);
            ...
            points[(int)PiePoints.TopLabelLine] = new Point3D(
                rectangle.Left + (float)Math.Cos((startAngle + sweepAngle / 2) * Math.PI / 180) * rectangle.Width / 2F + rectangle.Width / 2F,
                rectangle.Top + (float)Math.Sin((startAngle + sweepAngle / 2) * Math.PI / 180) * rectangle.Height / 2F + rectangle.Height / 2F,
                pieWidth);
            ...
            points[(int)PiePoints.TopLabelLineout] = new Point3D(
                rectangle.Left + (float)Math.Cos((startAngle + sweepAngle / 2) * Math.PI / 180) * rectangle.Width * sizeOut / 2F + rectangle.Width / 2F,
                rectangle.Top + (float)Math.Sin((startAngle + sweepAngle / 2) * Math.PI / 180) * rectangle.Height * sizeOut / 2F + rectangle.Height / 2F,
                pieWidth);
            ...
            if (Doughnut)
            {
                points[(int)PiePoints.TopLabelCenter] = new Point3D(
                    rectangle.Left + (float)Math.Cos((startAngle + sweepAngle / 2) * Math.PI / 180) * rectangle.Width * (1 + doughnutRadius) / 4F + rectangle.Width / 2F,
                    rectangle.Top + (float)Math.Sin((startAngle + sweepAngle / 2) * Math.PI / 180) * rectangle.Height * (1 + doughnutRadius) / 4F + rectangle.Height / 2F,
                    pieWidth);
            }
            else
            {
                points[(int)PiePoints.TopLabelCenter] = new Point3D(
                    rectangle.Left + (float)Math.Cos((startAngle + sweepAngle / 2) * Math.PI / 180) * rectangle.Width * 0.5F / 2F + rectangle.Width / 2F,
                    rectangle.Top + (float)Math.Sin((startAngle + sweepAngle / 2) * Math.PI / 180) * rectangle.Height * 0.5F / 2F + rectangle.Height / 2F,
                    pieWidth);
            }

            // Top Rectangle Top Left Point
            points[(int)PiePoints.TopRectTopLeftPoint] = new Point3D(rectangle.Left, rectangle.Top, pieWidth);
            ...
            points[(int)PiePoints.BottomRectTopLeftPoint] = new Point3D(rectangle.Left, rectangle.Top, 0);
            ...
            if (Doughnut)
            {
                ...
                points[(int)PiePoints.DoughnutTop180] = new Point3D(
                    rectangle.Left + (float)Math.Cos(180 * Math.PI / 180) * rectangle.Width * doughnutRadius / 2F + rectangle.Width / 2F,
                    rectangle.Top + (float)Math.Sin(180 * Math.PI / 180) * rectangle.Height * doughnutRadius / 2F + rectangle.Height / 2F,
                    pieWidth);
                ...
                points[(int)PiePoints.DoughnutTop0] = new Point3D(
                    rectangle.Left + (float)Math.Cos(0 * Math.PI / 180) * rectangle.Width * doughnutRadius / 2F + rectangle.Width / 2F,
                    rectangle.Top + (float)Math.Sin(0 * Math.PI / 180) * rectangle.Height * doughnutRadius / 2F + rectangle.Height / 2F,
                    pieWidth);
                ...
                points[(int)PiePoints.DoughnutTopStart] = new Point3D(
                    rectangle.Left + (float)Math.Cos(startAngle * Math.PI / 180) * rectangle.Width * doughnutRadius / 2F + rectangle.Width / 2F,
                    rectangle.Top + (float)Math.Sin(startAngle * Math.PI / 180) * rectangle.Height * doughnutRadius / 2F + rectangle.Height / 2F,
                    pieWidth);

                // Top End Angle point on the Doughnut arc
                points[(int)PiePoints.DoughnutTopEnd] = new Point3D(
                    rectangle.Left + (float)Math.Cos((startAngle + sweepAngle) * Math.PI / 180) * rectangle.Width * doughnutRadius / 2F + rectangle.Width / 2F,
                    rectangle.Top + (float)Math.Sin((startAngle + sweepAngle) * Math.PI / 180) * rectangle.Height * doughnutRadius / 2F + rectangle.Height / 2F,
                    pieWidth);
                ...
                points[(int)PiePoints.DoughnutTopRectTopLeftPoint] = new Point3D(rectangle.Left, rectangle.Top, pieWidth);
                ...
                points[(int)PiePoints.DoughnutBottomRectTopLeftPoint] = new Point3D(rectangle.Left, rectangle.Top, 0);
            ...
            {
                result[pointIndx] = point.SKPoint;
            ...
            )
        {
            ...
            {
                brush = ChartGraphics.GetGradientBrush(graph.GetAbsoluteRectangle(area.Position.ToSKRect()), Color.FromArgb((int)brushWithoutLight.Color.Alpha, 0, 0, 0), brushWithoutLight.Color, GradientStyle.VerticalCenter);
            }
        ...
        internal class LabelColumn
        {
            ...
            private SKRect _chartAreaPosition;

            private SKRect _innerPlotPosition;
            ...
            public LabelColumn(SKRect position)
            {
            ...
            internal int GetLabelIndex(float y)
            {
                ...
                if (y < _chartAreaPosition.Top)
                {
                    y = _chartAreaPosition.Top;
                }
                ...
                return (int)((y - _chartAreaPosition.Top) / columnHeight);
            }
            ...
            internal SKPoint GetLabelPosition(DataPoint dataPoint)
            {
                SKPoint position = SKPoint.Empty;
                int pointIndex = 0;
            ...
            internal void Initialize(SKRect rectangle, bool rightPosition, int maxNumOfRows, float labelLineSize)
            {
        ...
        {
            labelColumnLeft = new LabelColumn(area.Position.ToSKRect());
            labelColumnRight = new LabelColumn(area.Position.ToSKRect());
            ...
            foreach (DataPoint point in dataPoints)
            {
                ...
                SKSize size = graph.MeasureStringRel(GetLabelText(point).Replace("\\n", "\n"), point.Font);
            ...
            pieRectangle.Size = new(pieRectangle.Width - 2F * maxSize - 2 * pieRectangle.Width * labelLineSize,
            ...
            {
                pieRectangle.Size = new(oldWidth * (float)MinimumRelativePieSize(area), pieRectangle.Height);
            }

            // Size of pie chart can not be less then MinimumRelativePieSize of chart area.
            if (pieRectangle.Height < oldHeight * (float)MinimumRelativePieSize(area))
            {
                pieRectangle.Size = new(pieRectangle.Width, oldHeight * (float)MinimumRelativePieSize(area));
            }
            ...
            )
        {
            ...
            SKPoint[] piePoints = GetPiePoints(graph, area, pieWidth, pieRectangle, startAngle, sweepAngle, false, doughnutRadius, exploded);
        ...
        private static void Draw3DInsideLabels(ChartGraphics graph, SKPoint[] points, DataPoint point, int pointIndex)
        {
            ...
            // Get label relative position
            SKPoint labelPosition = graph.GetRelativePoint(points[(int)PiePoints.TopLabelCenter]);
