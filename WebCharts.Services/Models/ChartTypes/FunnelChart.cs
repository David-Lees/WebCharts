...
namespace WebCharts.Services.Models.ChartTypes
{
	...
	internal class FunnelChart : IChartType
	{
        ...
        internal SKRect plotAreaSpacing = new SKRect(3f, 3f, 3f, 3f);
        ...
        private SKRect _plotAreaPosition = SKRect.Empty;

		// Funnel chart drawing style
		private	FunnelStyle		_funnelStyle = FunnelStyle.YIsHeight;

		// Define the shape of the funnel neck
		private	SKSize			_funnelNeckSize = new SKSize(50f, 30f);
        ...
        /// </summary>
        internal SKRect PlotAreaPosition
        {
		...
		protected virtual void GetPointWidthAndHeight(
			...
			out float endWidth)
		{
			SKPoint	pointPositionAbs = SKPoint.Empty;

			// Get plotting area position in pixels
			SKRect plotAreaPositionAbs = this.Graph.GetAbsoluteRectangle(this.PlotAreaPosition);
		...
		private void DrawLabels()
		{
			...
			foreach(FunnelPointLabelInfo labelInfo in this.labelInfoList)
			{
					...
					!float.IsNaN(labelInfo.Position.Height) )
				{
					...
					SKSize spacing = this.Graph.MeasureString(
						"W",
		...
		private ArrayList CreateLabelsInfoList()
		{
			...
			SKRect plotAreaPositionAbs = this.Graph.GetAbsoluteRectangle( this.Area.Position.ToSKRect() );
		...
		private bool FitPointLabels()
		{
			...
			SKRect plotAreaPositionAbs = this.Graph.GetAbsoluteRectangle(PlotAreaPosition);
			...
			SKRect requiredSpacing = this.Graph.GetAbsoluteRectangle( new SKRect(1f, 1f, 1f, 1f) );
			foreach(FunnelPointLabelInfo labelInfo in this.labelInfoList)
			{
		...
		private void GetLabelsPosition()
		{
			...
			SKSize labelSpacing = new SKSize(3f, 3f);
		...
		private void GetPlotAreaSpacing()
		{
			...
			this.plotAreaSpacing = new SKRect(1f, 1f, 1f, 1f);
		...
		{
			// Set default gap size
			SKSize	neckSize = new SKSize(5f, 5f);
        ...
        private float GetFunnelPointGap(DataPointCustomProperties properties)
		{
            ...
            if (attrValue != null && attrValue.Length > 0)
            {
                ...
                // Convert from relative coordinates to pixels
                gapSize = this.Graph.GetAbsoluteSize(new SKSize(gapSize, gapSize)).Height;
            }
