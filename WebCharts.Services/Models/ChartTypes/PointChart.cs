...
namespace WebCharts.Services
{
    ...
    internal class PointChart : IChartType
    {
            ...
            bool adjustForDrawing)
        {
            ...
            SKRect labPosition = SKRect.Empty;
            labPosition.Size = new SKSize(size.Width, size.Height);
            ...
            {
                labPosition.Left = position.X - size.Width;
                if (adjustForDrawing && !pixelSize.IsEmpty)
                {
                    labPosition.Left -= 4f * pixelSize.Width;
                    labPosition.Right += 4f * pixelSize.Width;
                }
            }
            else if (format.Alignment == StringAlignment.Near)
            {
                labPosition.Left = position.X;
                if (adjustForDrawing && !pixelSize.IsEmpty)
                {
                    labPosition.Right += 4f * pixelSize.Width;
                }
            ...
            {
                labPosition.Left = position.X - size.Width / 2F;
                if (adjustForDrawing && !pixelSize.IsEmpty)
                {
                    labPosition.Left -= 2f * pixelSize.Width;
                    labPosition.Right += 4f * pixelSize.Width;
                }
            }
            ...
            {
                labPosition.Top = position.Y - size.Height;
            }
            else if (format.LineAlignment == StringAlignment.Near)
            {
                labPosition.Top = position.Y;
            }
            else if (format.LineAlignment == StringAlignment.Center)
            {
                labPosition.Top = position.Y - size.Height / 2F;
            }

            labPosition.Top -= 1f * pixelSize.Height;
            ...
            )
        {
            ...
            marker3DPosition[0] = new Point3D(markerPosition.X, markerPosition.Y, pointEx.zPosition + (middleMarker ? pointEx.depth / 2f : pointEx.depth));
            ...
            if ((alwaysDrawMarkers ||
                pointMarkerStyle != MarkerStyle.None ||
                pointMarkerImage.Length > 0) && (pointEx.index % ser.MarkerStep) == 0)
            {
                ...
                rectPath = graph.DrawMarker3D(area.matrix3D,
                    ...
                    (point.series != null) ? point.series.ShadowOffset : 0,
                    (point.series != null) ? point.series.ShadowColor : SKColor.Empty,
                    new SKRect(markerRotatedPosition.X, markerRotatedPosition.Y, markerSize.Width, markerSize.Height),
            ...
            if (common.ProcessModeRegions)
            {
                ...
                {
                    common.HotRegionsList.AddHotRegion(
                        new SKRect(markerRotatedPosition.X - relativeMarkerSize.Width / 2f, markerRotatedPosition.Y - relativeMarkerSize.Height / 2f, relativeMarkerSize.Width, relativeMarkerSize.Height),
                        point,
                        ser.Name,
                        pointEx.index - 1
                        );
        ...
        internal class Label3DInfo
        {
            ...
            internal SKPoint MarkerPosition = SKPoint.Empty;
