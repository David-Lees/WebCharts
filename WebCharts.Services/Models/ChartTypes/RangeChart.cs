...
namespace WebCharts.Services.Models.ChartTypes
{
    ...
    internal class RangeChart : SplineChart
    {
        ...
        /// </summary>
        internal SKPath areaBottomPath = new();
        ...
        /// </summary>
        internal SKPoint[] lowPoints = null;
        ...
        private void FillLastSeriesGradient(ChartGraphics graph)
        {
            ...
            if (gradientFill && areaPath != null)
            {
                ...
                using (SKPath gradientPath = new())
                {
                    ...
                    // Create brush
                    using SKPaint areaGradientBrush = ChartGraphics.GetGradientBrush(gradientPath.GetBounds(), _series.Color, _series.BackSecondaryColor, _series.BackGradientStyle);
            ...
            float tension)
        {
            ...
            SKPoint highPoint1 = points[pointIndex - 1];
            SKPoint highPoint2 = points[pointIndex];
            SKPoint lowPoint1 = lowPoints[pointIndex - 1];
            SKPoint lowPoint2 = lowPoints[pointIndex];
            ...
            if (graph.ActiveRenderingType == RenderingType.Svg)
            {
                ...
                {
                    pathReverse.AddPath(SkiaSharpExtensions.CreateSpline(lowPoints));

                    // Reversed points order in the aray
                    SKPoint[] pointsReversed = pathReverse.Points;
                    SKPoint[] newPoints = new SKPoint[pointsReversed.Length];
                    int pntIndex = pointsReversed.Length - 1;
                    foreach (SKPoint pp in pointsReversed)
                    {
                        newPoints[pntIndex] = pp;
                        pntIndex--;
                    }
                    ...
                    {
                        SKPoint[] newSKPoint = new SKPoint[3];
                        newSKPoint[0] = newPoints[0];
                        newSKPoint[1] = newPoints[1];
                        newSKPoint[2] = newPoints[1];
                        newPoints = newSKPoint;
                    }

                    // Add Polygon to the path
                    path.AddPoly(newPoints);
                }
            }
            else
            {
            ...
            // Draw shadow
            if (series.ShadowColor != SKColor.Empty && series.ShadowOffset != 0 && point.Color != SKColor.Empty && point.Color != SKColors.Transparent)
            {
                // Translate drawing matrix
                SKMatrix translateMatrix = graph.Transform;
                translateMatrix.TransX = series.ShadowOffset;
                translateMatrix.TransX = series.ShadowOffset;
                SKMatrix oldMatrix = graph.Transform;
                graph.Transform = translateMatrix;

                SKRegion shadowRegion = new(path);
                using SKPaint shadowBrush = new() { Color = (series.ShadowColor.Alpha != 255) ? series.ShadowColor : Color.FromArgb(point.Color.Alpha / 2, series.ShadowColor), Style = SKPaintStyle.Fill };
                ...
                using SKPaint areaLinePen = new() { Color = shadowBrush.Color, StrokeWidth = 1, Style = SKPaintStyle.Stroke };
                if (pointIndex == 0)
                {
            ...
            if (!gradientFill)
            {
                ...
                {
                    SKPaint areaLinePen = new() { Color = areaBrush.Color, StrokeWidth = 1, Style = SKPaintStyle.Stroke };
            ...
            {
                areaPath = new SKPath();
                areaPath.MoveTo(highPoint1.X, lowPoint1.Y);
                areaPath.LineTo(highPoint1.X, highPoint1.Y);
            }
            ...
            {
                areaPath.AddPath(SkiaSharpExtensions.CreateSpline(points));
            }
            ...
            {
                areaBottomPath.AddPath(SkiaSharpExtensions.CreateSpline(lowPoints));
            }
            ...
            if (common.ProcessModeRegions)
            {
                ...
                {
                    mapAreaPath.AddPath(SkiaSharpExtensions.CreateSpline(points));
                }
                ...
                {
                    mapAreaPath.AddPath(SkiaSharpExtensions.CreateSpline(lowPoints));
                }

                float[] coord = new float[mapAreaPath.PointCount * 2];
                SKPoint[] pathPoints = mapAreaPath.Points;
                for (int i = 0; i < mapAreaPath.PointCount; i++)
                {
                    // Allocate array of floats
                    SKPoint pointNew = graph.GetRelativePoint(pathPoints[i]);
            ...
            bool clipOnBottom)
        {
            ...
            if (Math.Abs(firstPoint.yPosition - thirdPoint.Y) > smallDouble &&
                Math.Abs(secondPoint.yPosition - fourthPoint.Y) > smallDouble &&
                ...
                (firstPoint.yPosition < thirdPoint.Y && secondPoint.yPosition > fourthPoint.Y)))
            {
                ...
                SKPoint intersectionCoordinates = ChartGraphics.GetLinesIntersection(
            ...
            out SKPoint fourthPoint)
        {
            ...
            thirdPoint = new SKPoint((float)firstPoint.xPosition, secondYValue);
            secondYValue = (float)vAxis.GetPosition(secondPoint.dataPoint.YValues[1]);
            fourthPoint = new SKPoint((float)secondPoint.xPosition, secondYValue);
        ...
        /// <returns>Returns elemnt shape path if operationType parameter is set to CalcElementPath, otherwise Null.</returns>
        internal static SKPath Draw3DSplinePolygon(
            ChartGraphics graph,
            ChartArea area,
            float positionZ,
            SKColor backColor,
            ...
            bool forceThinBorder)
        {
            ...
            SKColor polygonColor = area.matrix3D.GetPolygonLight(points3D, backColor, topIsVisible, area.Area3DStyle.Rotation, SurfaceNames.Front, area.ReverseSeriesOrder);
            ...
            if (drawElements)
            {
                ...
                if (forceThinBorder)
                {
                    graph.DrawPath(new SKPaint() { Color = surfaceBorderColor, StrokeWidth = 1, Style = SKPaintStyle.Stroke }, resultPath);
                }
                else if (polygonColor.Alpha == 255)
                {
                    graph.DrawPath(new SKPaint() { Color = polygonColor, StrokeWidth = 1, Style = SKPaintStyle.Stroke }, resultPath);
                }
                ...
                SKPaint thickBorderPen = new() { Style = SKPaintStyle.Stroke, Color = surfaceBorderColor, StrokeWidth = borderWidth };
                ...
                {
                    graph.DrawLine(thickBorderPen, topLine.Points.Last(), bottomLine.Points[0]);
                }
            }
