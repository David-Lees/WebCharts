...
namespace WebCharts.Services.Models.ChartTypes
{
	...
	internal class RangeChart : SplineChart
	{
		...
		/// </summary>
        internal SKPath areaBottomPath = new SKPath();
			...
			float tension)
		{
			...
			{
                using (SKPath pathReverse = new SKPath())
                {
                    ...
                    else
                    {
                        ...
                        SKPoint[] pointsReversed = pathReverse.PathPoints;
                        SKPoint[] SKPoint = new SKPoint[pointsReversed.Length];
                        ...
                        if (SKPoint.Length == 2)
                        {
                            ...
                            newSKPoint[0] = SKPoint[0];
                            newSKPoint[1] = SKPoint[1];
                            newSKPoint[2] = SKPoint[1];
                            SKPoint = newSKPoint;
                        }

                        // Add Polygon to the path
                        path.AddPolygon(SKPoint);
                    }
                }
			}
			else
			...
			if( common.ProcessModeRegions )
			{
				...
				SKPoint[] pathPoints = mapAreaPath.PathPoints;
			...
			bool clipOnBottom)
		{
			...
			//****************************************************************
			SKPoint	thirdPoint, fourthPoint; 
		...
		/// <returns>Returns elemnt shape path if operationType parameter is set to CalcElementPath, otherwise Null.</returns>
		internal SKPath Draw3DSplinePolygon( 
			ChartGraphics graph, 
			ChartArea area,
			float positionZ, 
			Color backColor, 
