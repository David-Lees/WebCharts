...
namespace WebCharts.Services
{
    ...
    public abstract class Annotation : ChartNamedElement
    {
        ...
        internal SKColor lineColor = SKColors.Black;
        ...
        internal SKPoint lastPlacementPosition = SKPoint.Empty;

        // Relative location of annotation anchor, when it's started to move
        internal SKPoint startMoveAnchorLocationRel = SKPoint.Empty;

        // Relative position of annotation, when it's started to move/resize
        internal SKRect startMovePositionRel = SKRect.Empty;

        // Relative position of annotation, when it's started to move/resize
        internal SKPath startMovePathRel = null;
        ...
        virtual public SKColor ForeColor
        {
            get
            {
                return _foreColor;
        ...
        virtual public SKColor LineColor
        {
            get
            {
                return lineColor;
        ...
        virtual public SKColor BackColor
        {
            get
            {
                return _backColor;
        ...
        virtual public SKColor BackSecondaryColor
        {
            get
            {
        ...
        virtual public SKColor ShadowColor
        {
            get
            {
                return _shadowColor;
        ...
        virtual internal void PaintSelectionHandles(ChartGraphics chartGraphics, SKRect rect, SKPath path)
        {
            // Define markers appearance style
            SKColor markerBorderColor = SKColors.Black;
            SKColor markerColor = new(255, 255, 255, 200);
            MarkerStyle markerStyle = MarkerStyle.Square;
            int markerSize = selectionMarkerSize;
            var selected = IsSelected;

            SKSize markerSizeRel = chartGraphics.GetRelativeSize(new SKSize(markerSize, markerSize));
                ...
                !Common.ChartPicture.isPrinting)
            {
                ...
                if (selected)
                {
                    ...
                    if (SelectionPointsStyle == SelectionPointsStyle.TwoPoints)
                    {
                        ...
                        selectionRects[(int)ResizingMode.TopLeftHandle] = new SKRect(
                            rect.Left - markerSizeRel.Width / 2f,
                            rect.Top - markerSizeRel.Height / 2f,
                            markerSizeRel.Width,
                            markerSizeRel.Height);
                        selectionRects[(int)ResizingMode.BottomRightHandle] = new SKRect(
                    ...
                    else if (SelectionPointsStyle == SelectionPointsStyle.Rectangle)
                    {
                        ...
                        {
                            // Get handle position
                            SKPoint handlePosition = SKPoint.Empty;
                    ...
                    if (!double.IsNaN(anchorX) && !double.IsNaN(anchorY))
                    {
                        ...
                        chartGraphics.DrawMarkerRel(
                            new SKPoint((float)anchorX, (float)anchorY),
                            MarkerStyle.Cross,
        ...
        virtual internal SKRect GetContentPosition()
        {
        ...
        virtual internal void GetRelativePosition(out SKPoint location, out SKSize size, out SKPoint anchorLocation)
        {
            ...
            //***********************************************************************
            location = new SKPoint((float)relativeX, (float)relativeY);
            size = new SKSize((float)relativeWidth, (float)relativeHeight);
            anchorLocation = new SKPoint((float)anchorX, (float)anchorY);
            ...
            {
                currentPositionRel = new SKRect(location.X, location.Y, location.X + size.Width, location.Y + size.Height);
                currentAnchorLocationRel = new SKPoint(anchorLocation.X, anchorLocation.Y);
            }
        }
        ...
        internal void SetPositionRelative(SKRect position, SKPoint anchorPoint)
        {
		...
		virtual internal void AdjustLocationSize(SKSize movingDistance, ResizingMode resizeMode)
        {
		...
		virtual internal void AdjustLocationSize(SKSize movingDistance, ResizingMode resizeMode, bool pixelCoord)
        {
        ...
        /// <param name="pixelCoord">Distance is in pixels, otherwise relative.</param>
        /// <param name="userInput">Indicates if position changing was a result of the user input.</param>
		virtual internal void AdjustLocationSize(SKSize movingDistance, ResizingMode resizeMode, bool pixelCoord, bool userInput)
        {
        ...
        internal ResizingMode GetSelectionHandle(SKPoint point)
        {
    ...
    public class AnnotationPositionChangingEventArgs : EventArgs
    {
        ...
        /// </summary>
        public SKPoint NewAnchorLocation
        {
            get
            {
                return new SKPoint(
