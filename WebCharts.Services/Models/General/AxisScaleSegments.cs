...
namespace WebCharts.Services.Models.General
{
     ...
     internal class AxisScaleSegment
	{
		...
		internal void PaintBreakLine(ChartGraphics graph, AxisScaleSegment nextSegment)
		{
			...
			SKRect breakPosition = this.GetBreakLinePosition(graph, nextSegment);

			// Get top line graphics path
			SKPath breakLinePathTop = this.GetBreakLinePath(breakPosition, true);
				...
				using(SKPath fillPath = new SKPath())
				{
					...
					using(Brush fillBrush = this.GetChartFillBrush(graph))
					{
						...
						if( this.axis.ChartArea.ShadowOffset != 0 && !this.axis.ChartArea.ShadowColor.IsEmpty)
						{
							...
							// Draw new shadow
							using(SKPath shadowPath = new SKPath())
							{
								...
								SKRect clipRegion = graph.GetAbsoluteRectangle(this.axis.PlotAreaPosition.ToSKRect());
								if( this.axis.AxisPosition == AxisPosition.Right || this.axis.AxisPosition == AxisPosition.Left )
								{
		...
		private Brush GetChartFillBrush(ChartGraphics graph)
		{
			...
			else
			{
                ...
                brush = graph.GetGradientBrush(new SKRect(0, 0, chart.chartPicture.Width - 1, chart.chartPicture.Height - 1), chart.BackColor, chart.BackSecondaryColor, chart.BackGradientStyle);
			}
		...
		private SKPath GetBreakLinePath(SKRect breakLinePosition, bool top)
		{
			...
			if(this.axis.ScaleBreakStyle.BreakLineStyle == BreakLineStyle.Wave)
			{
				...
				if( this.axis.AxisPosition == AxisPosition.Right || this.axis.AxisPosition == AxisPosition.Left )
				{
					...
					points[points.Length - 1] = new SKPoint(endX, y);
				}
				else
				{
					...
					points[points.Length - 1] = new SKPoint(x, endY);
				}
		...
		{
			// Start with the plotting rectangle position
			SKRect breakPosition = this.axis.PlotAreaPosition.ToSKRect();
