...
using WebCharts.Services.Models.Converters;
...
namespace WebCharts.Services.Models.General
{
	...
	public class Title : ChartNamedElement, IDisposable
    {
		...
		internal void Invalidate(bool invalidateTitleOnly)
		{
			if(Chart != null)
			{
				...
				if(invalidateTitleOnly)
				{
					...
					SKRect	invalRect = Chart.ClientRectangle;
        ...
        internal void Paint(ChartGraphics chartGraph )
		{
			...
			//***************************************************************
			SKRect	titlePosition = this.Position.ToSKRect();
			...
			if(!this.Position.Auto && Common != null && Common.ChartPicture != null)
			{
				if(titlePosition.Width == 0 || titlePosition.Height == 0)
				{
					...
					if(titlePosition.Width == 0)
					{
						...
						{
							titlePosition.Left = titlePosition.Left - titlePosition.Width;
						}
			...
			//***************************************************************
			SKRect	absPosition = new SKRect(titlePosition.Location, titlePosition.Size);
			...
			else 
			{
				...
				SKRect exactTitleRect = new SKRect(
					titlePosition.Left, 
					titlePosition.Top,
			...
			if(this.BackGroundIsVisible)
			{
				absPosition.Right -= this.titleBorderSpacing;
				absPosition.Bottom -= this.titleBorderSpacing;
				absPosition.Left += this.titleBorderSpacing / 2f;
				absPosition.Top += this.titleBorderSpacing / 2f;
			}
            ...
            if (titleText.Length > 0)
            {
                ...
                else if (textStyle == TextStyle.Frame)
                {
                    using (SKPath SKPath = new SKPath())
                    {
                        ...
                        {
                            chartGraph.DrawPath(pen, SKPath);
                        }
                    }
                }
                ...
                {
                    // Draw shadow
                    SKRect shadowPosition = new SKRect(absPosition.Location, absPosition.Size);
                    shadowPosition.X -= 1;
                    shadowPosition.Y -= 1;
			...
			float elementSpacing)
		{
			...
			// Get title size
			SKRect		titlePosition = new SKRect();
			SKSize			layoutArea = new SKSize(chartAreasRectangle.Width, chartAreasRectangle.Height);
			...
			if(this.Docking == Docking.Left)
			{
				...
				titlePosition.Bottom = titlePosition.Top + chartAreasRectangle.Bottom - titlePosition.Y - elementSpacing;
				if(titlePosition.Height < 0)
				{
