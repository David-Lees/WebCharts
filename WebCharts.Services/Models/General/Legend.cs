...
namespace WebCharts.Services.Models.General
{
    ...
    public class Legend : ChartNamedElement
	{
		...
		{
			SKRect	legendPosition = new SKRect();

			// Get optimal legend size
			SKSize maxSize = new SKSize(chartAreasRectangle.Width - 2*elementSpacing, chartAreasRectangle.Height - 2*elementSpacing);
			...
			SKSize legendSize = this.GetOptimalSize(chartGraph, maxSize);
        ...
        {
            Common.HotRegionsList.AddHotRegion(this.Position.ToSKRect(), this, ChartElementType.LegendArea, true);
        }
		...
		internal void Paint(ChartGraphics chartGraph )
		{
			...
			if( Common.ProcessModePaint )
			{
				...
				chartGraph.FillRectangleRel( 
					chartGraph.GetRelativeRectangle(Rectangle.Round(chartGraph.GetAbsoluteRectangle(this.Position.ToSKRect()))),
					BackColor, 
					BackHatchStyle, 
					BackImage, 
					BackImageWrapMode, 
			...
			{
				Rectangle separatorRect = Rectangle.Round(chartGraph.GetAbsoluteRectangle(this.Position.ToSKRect()));
		...
		private void DrawLegendHeader(ChartGraphics chartGraph)
		{
				...
				this._headerPosition.Height > 0)
			{
				...
				// Get Legend position
				Rectangle legendPosition = Rectangle.Round(chartGraph.GetAbsoluteRectangle(this.Position.ToSKRect()));
		...
		private LegendTableStyle GetLegendTableStyle(ChartGraphics chartGraph)
		{
			...
			if(this.TableStyle == LegendTableStyle.Auto)
			{
				...
				else
				{
					...
					SKSize legendPixelSize = chartGraph.GetAbsoluteRectangle(this.Position.ToSKRect()).Size;
