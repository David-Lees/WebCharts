    ...
    public partial class ChartGraphics : ChartElement
    {
            ...
            )
        {
                ...
                backColor != SKColors.Transparent)
            {
                ...
                using (Graphics graphics = Graphics.FromImage(bitmap))
                {
                    using (SolidBrush backBrush = new SolidBrush(backColor))
                    {
                        ...
                        backFilledBrush = new TextureBrush(bitmap, new SKRect(0, 0, image.Width, image.Height), attrib);
                    }
                }
            ...
            if (ImageLoader.DoDpisMatch(image, this.Graphics))
                textureBrush = new TextureBrush(image, new SKRect(0, 0, image.Width, image.Height), attrib);
            else  // if the image dpi does not match the graphics dpi we have to scale the image    
            {
                ...
                textureBrush = new TextureBrush(scaledImage, new SKRect(0, 0, scaledImage.Width, scaledImage.Height), attrib);
                scaledImage.Dispose();
            }
        ...
        internal SKPaint GetGradientBrush(
            SKRect rectangle,
            SKColor firstColor,
            SKColor secondColor,
            GradientStyle type
        ...
        internal SKPoint[] CreateStarPolygon(SKRect rect, int numberOfCorners)
        {
            ...
            {
                tempPoints[0] = new SKPoint(rect.X + rect.Width / 2f, (outside == true) ? rect.Y : rect.Y + rect.Height / 4f);
                Matrix matrix = new Matrix();
                matrix.RotateAt(pointIndex * (360f / (numberOfCorners * 2f)), new SKPoint(rect.X + rect.Width / 2f, rect.Y + rect.Height / 2f));
                                    ...
                                    else
                                    {
                                        ...
                                        using (SKPath path = new SKPath())
                                        {
                                            ...
                                            using (PathGradientBrush shadowBrush = new PathGradientBrush(path))
                                            {
                                                ...
                                                shadowBrush.CenterPoint = new SKPoint(point.X, point.Y);

                                                // Define brush focus scale
                                                SKPoint focusScale = new SKPoint(1 - 2f * shadowSize / rect.Width, 1 - 2f * shadowSize / rect.Height);
                                                if (focusScale.X < 0)
                                                {
                                                    focusScale.X = 0;
                                                }
        ...
        internal SKSize MeasureStringRel(
            string text,
            Font font,
            SKSize layoutArea,
            StringFormat stringFormat,
            ...
            int pointIndex)
        {
            ...
            {
                SKRect backPositionAbs = this.Round(this.GetAbsoluteRectangle(backPosition));

                // Get rotation point
                SKPoint rotationPoint = SKPoint.Empty;
                if (textPosition.IsEmpty)
                {
                    rotationPoint = new SKPoint(backPositionAbs.X + backPositionAbs.Width / 2f, backPositionAbs.Y + backPositionAbs.Height / 2f);
                }
                else
                {
            ...
            bool truncatedRight)
        {
            ...
            using (StringFormat drawingFormat = (StringFormat)format.Clone())
            {
                ...
                SKRect absPosition = this.GetAbsoluteRectangle(position);
                ...
                //********************************************************************
                SKRect labelRect = Rectangle.Empty;
                float offsetY = 0f;
                float offsetX = 0f;
                ...
                if (angle != 0 && angle != 90 && angle != -90)
                {
                    ...
                    if (boundaryRect != SKRect.Empty)
                    {
                        ...
                        if (!region.IsEmpty(Graphics))
                        {
                            ...
                            SKRect truncateRect = region.GetBounds(Graphics);
                ...
                //********************************************************************
                SKRect absPositionWithoutImage = new SKRect(absPosition.Location, absPosition.Size);
                ...
                if (labelRowIndex > 0 && labelMark != LabelMarkStyle.None)
                {
                    ...
                    SKSize labelSizeWithImage = new SKSize(labelSize.Width, labelSize.Height);
                    if (labelImage != null)
                    {
            ...
            Matrix originalTransform)
        {
            ...
            SKPoint centerNotRound = new SKPoint(absPosition.X + absPosition.Width / 2F, absPosition.Y + absPosition.Height / 2F);
            ...
            Matrix oldTransform)
        {
            ...
            {
                // Calculate center of the text rectangle
                System.Drawing.Point center = System.Drawing.Point.Round(new SKPoint(absPosition.X + absPosition.Width / 2F, absPosition.Y + absPosition.Height / 2F));
        ...
        /// <param name="font">The Font object used to determine the size of the text string. </param>
        /// <returns>A SKSize structure that represents the size of text as drawn with font.</returns>
        internal SKSize MeasureStringRel(string text, Font font)
        {
            SKSize newSize;
        ...
        /// <param name="stringFormat">A StringFormat object that represents formatting information, such as line spacing, for the text string. </param>
        /// <returns>A SKSize structure that represents the size of text as drawn with font.</returns>
        internal SKSize MeasureStringRel(string text, Font font, SKSize layoutArea, StringFormat stringFormat)
        {
            SKSize size, newSize;
        ...
        {
            // Measure string
            SKSize size = this.MeasureString(text, font);
        ...
        /// <param name="stringFormat">A StringFormat object that represents formatting information, such as line spacing, for the text string. </param>
        /// <returns>A SKSize structure that represents the size of text as drawn with font.</returns>
        internal Size MeasureStringAbs(string text, Font font, SKSize layoutArea, StringFormat stringFormat)
        {
        ...
        internal void DrawStringRel(string text, Font font, Brush brush, SKRect layoutRectangle, StringFormat format)
        {
            SKRect rect;
            ...
            bool isVertical)
        {
                ...
                borderWidth > 0)
            {
                ...
                if (this.ActiveRenderingType == RenderingType.Svg ||
                    this.IsMetafile)
                {
                    fillRect = new SKRect(rect.X, rect.Y, rect.Width, rect.Height);
                }
            ...
            int circularSectorsCount)
        {
            ...
            else
            {
                ...
                Color[] colors = { Color.Transparent };
                shadowBrush.SurroundColors = colors;
                shadowBrush.CenterPoint = new SKPoint(offset.X + offset.Width / 2f, offset.Y + offset.Height / 2f);

                // Define brush focus scale
                SKPoint focusScale = new SKPoint(1 - 2f * shadowOffset / offset.Width, 1 - 2f * shadowOffset / offset.Height);
                if (focusScale.X < 0)
                    focusScale.X = 0;
                if (focusScale.Y < 0)
                    focusScale.Y = 0;
        ...
        {
            SKPoint firstPoint = new SKPoint(position.X + position.Width / 2f, position.Y);
            SKPoint centerPoint = new SKPoint(position.X + position.Width / 2f, position.Y + position.Height / 2f);
            float sectorSize = 0f;
            SKPath path = new SKPath();
        ...
        internal void DrawCircleAbs(Pen pen, Brush brush, SKRect position, int polygonSectorsNumber, bool circle3D)
        {
        ...
        internal SKColor GetBrightGradientColor(SKColor beginColor, double position)
        {
            ...
            PenAlignment penAlignment)
        {
            ...
            // The fill rectangle is same
            fillRect = new SKRect(rect.X + borderWidth, rect.Y + borderWidth, rect.Width - borderWidth * 2, rect.Height - borderWidth * 2);
        ...
        internal SKPath CreateRoundedRectPath(SKRect rect, float[] cornerRadius)
        {
            // Create rounded rectangle path
            SKPath path = new SKPath();
        ...
        internal void DrawRoundedRectShadowAbs(SKRect rect, float[] cornerRadius, float radius, SKColor centerColor, SKColor surroundColor, float shadowScale)
        {
            ...
            shadowBrush.SurroundColors = colors;
            shadowBrush.CenterPoint = new SKPoint(rect.Left + rect.Width / 2f, rect.Top + rect.Height / 2f);

            // Define brush focus scale
            SKPoint focusScale = new SKPoint(1 - shadowScale * radius / rect.Width, 1 - shadowScale * radius / rect.Height);
            shadowBrush.FocusScales = focusScale;
            ...
            float doughnutRadius)
        {
            if (pieDrawingStyle == PieDrawingStyle.Concave)
            {
                ...
                using (SKPath brushPath = new SKPath())
                {
            ...
            else if (pieDrawingStyle == PieDrawingStyle.SoftEdge)
            {
                ...
                using (SKPath brushPath = new SKPath())
                {
                    ...
                    {
                        // Create brush path
                        using (SKPath brushInsidePath = new SKPath())
                        {
        ...
        /// <param name="backColor">Fill color</param>
        private void DrawPieSoftShadow(float startAngle, float sweepAngle, SKRect absRect, Color backColor)
        {
            SKPath path = new SKPath();
        ...
        internal void DrawArrowRel(SKPoint position, ArrowOrientation orientation, AxisArrowStyle type, Color color, int lineWidth, ChartDashStyle lineDashStyle, double shift, double size)
        {
        ...
        internal SKRect Round(SKRect rect)
        {
