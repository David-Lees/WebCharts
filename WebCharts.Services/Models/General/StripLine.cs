...
namespace WebCharts.Services.Models.General
{
	...
	public class StripLine : ChartElement
    {
			...
			bool drawLinesOnly)
		{
			...
			SKRect	plotAreaPosition = this.Axis.ChartArea.PlotAreaPosition.ToSKRect();
			...
			do
			{
                ...
                else if (this.StripWidth == 0 && drawLinesOnly)
                {
                    if (currentPosition > this.Axis.ViewMinimum && currentPosition < this.Axis.ViewMaximum)
                    {
                        ...
                        if (common.ProcessModeRegions)
                        {
                            SKSize relBorderWidth = new SKSize(this.BorderWidth + 1, this.BorderWidth + 1);
		...
		private void PaintTitle(ChartGraphics graph, SKRect rect)
		{
			if(this.Text.Length > 0)
			{
                ...
                using (StringFormat format = new StringFormat())
                {
                    ...
                    // Measure string size
                    SKSize size = graph.MeasureStringRel(titleText.Replace("\\n", "\n"), this.Font, new SKSize(100, 100), format, this.GetTextOrientation());
                    ...
                    SKSize sizeBorder = graph.GetRelativeSize(new SKSize(this.BorderWidth, this.BorderWidth));
