...
namespace WebCharts.Services.Models.General
{
    ...
    public class ChartElementOutline : IDisposable
    {
        ...
        {
            this.Markers = new ReadOnlyCollection<SKPoint>(new SKPoint[] { });
        }
        ...
        , IDisposable
    {
        ...
        internal HitTestResult[] HitTest(int x, int y, bool ignoreTransparent, params ChartElementType[] requestedElementTypes)
        {
            ...
            SKRect mouseRect = new SKRect(x - 1, y - 1, 2, 2);
            relativeX = this.Graph.GetRelativePoint(new SKPoint(x, y)).X;
            relativeY = this.Graph.GetRelativePoint(new SKPoint(x, y)).Y;
            SKRect relativeMouseRect = this.Graph.GetRelativeRectangle(mouseRect);
            ...
            for (int index = regionList.Count - 1; index >= 0; index--)
            {
                ...
                if (region.BoundingRectangle.IntersectsWith(newMouseRect))
                {
                    ...
                    if (region.Path != null)
                    {
                        ...
                        SKPathIterator iterator = new SKPathIterator(region.Path);
        ...
        internal ChartElementOutline GetChartElementOutline(object chartObject, ChartElementType elementType)
        {
            ...
            result.Markers = new ReadOnlyCollection<SKPoint>((SKPoint[])list.ToArray(typeof(SKPoint)));
            result.OutlinePath = GetSKPath(list, chartObject, elementType);
            return result;
        }
        ...
        /// <param name="elementType">Type of the element.</param>
        /// <returns></returns>
        private SKPath GetSKPath(IList markers, object chartObject, ChartElementType elementType)
        {
            ...
            {
                SKPath path = new SKPath();
                SKPoint[] points = new SKPoint[markers.Count];
                ...
                if (points.Length > 3)
                {
                    if (elementType == ChartElementType.DataPointLabel)
                    {
                        ...
                        {
                            SKRect rect = SKRect.FromLTRB(points[i].X, points[i].Y, points[i + 2].X, points[i + 2].Y);
                            path.Reset();
        ...
        private ArrayList GetMarkersFromRegions(object chartObject, ChartElementType elementType)
        {
            ...
            if (grid != null)
            {
                foreach (HotRegion rgn in regions)
                {
                    if (!IsArea3D(grid.Axis.ChartArea))
                    {
                        ...
                        {
                            SKPathIterator iterator = new SKPathIterator(rgn.Path);
                            ...
                            if (iterator.SubpathCount > 1)
                            {
                                ...
                                while (iterator.NextMarker(subPath) > 0)
                                {
                                    ...
                                    list.Add(new SKPoint(rect.Left + rect.Width / 2, rect.Top));
                                    list.Add(new SKPoint(rect.Right, rect.Top + rect.Height / 2));
                                    list.Add(new SKPoint(rect.Right - rect.Width / 2, rect.Bottom));
                                    list.Add(new SKPoint(rect.Left, rect.Bottom - rect.Height / 2));
                                    subPath.Reset();
                                }
                            }
                        }
                        ...
                        {
                            // 2D
                            rect = this.GetHotRegionRectangle(rgn, SKRect.Empty, elementType);
                            if (grid != null)
                            {
                    ...
                    {   // 3D
                        SKPoint[] points = rgn.Path.PathPoints;
                        ...
                        {   //Each gridline has a corresponding set of 4 points in the path
                            ...
                            SKPoint middleP0P3 = new SKPoint((points[i].X + points[i + 3].X) / 2f, (points[i].Y + points[i + 3].Y) / 2f);
                            SKPoint middleP1P2 = new SKPoint((points[i + 1].X + points[i + 2].X) / 2f, (points[i + 1].Y + points[i + 2].Y) / 2f);
            ...
            if (dataPoint != null && elementType != ChartElementType.DataPointLabel)
            {
                ...
                SKPoint pp = this.Transform3D(area, dataPoint);
        ...
        /// <returns>List of SKPoint.</returns>
        private ArrayList GetSeriesMarkers(Series series)
        {
            ...
            if (series != null)
            {
                ...
                if (ChartPicture.ChartAreas.IndexOf(areaName) != -1 && series.Enabled)
                {
                    ...
                    {
                        SKPoint pp = this.Transform3D(chartArea, point);
                        if (float.IsNaN(pp.X) || float.IsNaN(pp.Y))
                        {
                            continue;
                        }
        ...
        private ArrayList GetAxisMarkers(ChartGraphics graph, Axis axis)
        {
            ...
            SKRect rect1 = new SKRect(first.X, first.Y, second.X - first.X, second.Y - first.Y);
            ...
            if (this.IsArea3D(area))
            {
                ...
                {
                    points[i] = new Point3D(((SKPoint)list1[i]).X, ((SKPoint)list1[i]).Y, zPositon);
                }
                axis.ChartArea.matrix3D.TransformPoints(points);
                for (int i = 0; i < list1.Count; i++)
                {
                    list1[i] = points[i].SKPoint;
                }
            }
            foreach (SKPoint p in list1)
            {
        ...
        private ArrayList GetAreaMarkers(ChartGraphics graph, ChartArea area)
        {
            ...
            IList list1 = this.GetMarkers(area.PlotAreaPosition.ToSKRect(), ChartElementType.PlottingArea);
        ...
        /// <summary>
        /// Builds list of markers (SKPoint) based on rectangle
        /// </summary>
        /// <param name="rect">The rectangle</param>
        /// <param name="elementType">The type of chart elements to retrieve.</param>
        /// <returns>List of SKPoint</returns>
        private ArrayList GetMarkers(SKRect rect, ChartElementType elementType)
        {
            ...
            {
                SKSize relSize = this.Graph.GetRelativeSize(new SKSize(4f, 4f));
        ...
        /// <returns>List of SKPoint</returns>
        private ArrayList GetMarkers(SKRect rect, Boolean addAdditionalMarkers)
        {
            ArrayList list = new ArrayList();
            if (!addAdditionalMarkers)
            {
                ...
                {
                    list.Add(new SKPoint(rect.Left, rect.Top));
                    list.Add(new SKPoint(rect.Right, rect.Top));
                    list.Add(new SKPoint(rect.Right, rect.Bottom));
                    list.Add(new SKPoint(rect.Left, rect.Bottom));
                }
            ...
            else
            {
                if (rect.Width > 0)
                {
                    ...
                    if (rect.Width > 30)
                    {
                        list.Add(new SKPoint(rect.Left + rect.Width / 2, rect.Bottom));
                    }

                    list.Add(new SKPoint(rect.Left, rect.Bottom));
                    if (rect.Height > 30)
                    {
                        list.Add(new SKPoint(rect.Left, rect.Top + rect.Height / 2));
                    }
                }
        ...
        /// <returns>List of SKPoint.</returns>
        private ArrayList GetRegionMarkers(SKPath path)
        {
            return new ArrayList(path.PathPoints);
        }

        /// <summary>
        /// Calculates a DataPoint of 3D area into SKPoint to draw.
        /// </summary>
        /// <param name="chartArea">3D chart area</param>
        /// <param name="point">The DataPoint</param>
        /// <returns>Calculated SKPoint</returns>
        private SKPoint Transform3D(ChartArea chartArea, DataPoint point)
        {
            if (chartArea is ChartArea && IsArea3D(chartArea))
            {
                ...
                return annot3DPoints[0].SKPoint;
            }
        ...
        private SKRect GetHotRegionRectangle(HotRegion rgn, SKRect unionWith, ChartElementType elementType)
        {
            SKRect rect;
            if (rgn.Path != null)
            {
            ...
            {
                return SKRect.Union(unionWith, rect);
            }
            return rect;
        }
