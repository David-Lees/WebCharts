...
namespace WebCharts.Services.Models.General
{
	...
	public class Cursor : IDisposable
	{
		...
		private	SKPoint					_userSelectionStart = SKPoint.Empty;
		...
		internal void Paint( ChartGraphics graph )
		{
			...
			SKRect	plotAreaPosition = this._chartArea.PlotAreaPosition.ToSKRect();
		...
		/// </summary>
        /// <param name="point">A SKPoint structure that specifies where the cursor will be drawn.</param>
		...
		public void SetCursorPixelPosition(SKPoint point, bool roundToBoundary)
		{
			if(this._chartArea != null && this._chartArea.Common != null && this.GetAxis() != null)
			{
				SKPoint relativeCoord = GetPositionInPlotArea(point, roundToBoundary);
		...
		/// </summary>
        /// <param name="startPoint">A SKPoint structure that specifies where the range selection begins.</param>
        /// <param name="endPoint">A SKPoint structure that specifies where the range selection ends</param>
		...
		public void SetSelectionPixelPosition(SKPoint startPoint, SKPoint endPoint, bool roundToBoundary)
		{
			if(this._chartArea != null && this._chartArea.Common != null && this.GetAxis() != null)
			{
				...
				{
					SKPoint relativeCoord = GetPositionInPlotArea(startPoint, roundToBoundary);
					if(!relativeCoord.IsEmpty)
					{
				...
				if(!endPoint.IsEmpty)
				{
					SKPoint relativeCoord = GetPositionInPlotArea(endPoint, roundToBoundary);
					if(!relativeCoord.IsEmpty)
					{
		...
		internal void Cursor_MouseDown(object sender, System.Windows.Forms.MouseEventArgs e)
		{
			...
			if(e.Button == MouseButtons.Left && !GetPositionInPlotArea(new SKPoint(e.X, e.Y), false).IsEmpty)
			{
				// Change cursor position and selection start position when mouse down
				if(this.IsUserEnabled)
				{
					SetCursorPixelPosition(new SKPoint(e.X, e.Y), false);
				}
				if(this.IsUserSelectionEnabled)
				{
					this._userSelectionStart = new SKPoint(e.X, e.Y);
					SetSelectionPixelPosition(this._userSelectionStart, SKPoint.Empty, false);
				}
			}
		...
		internal void Cursor_MouseUp(object sender, System.Windows.Forms.MouseEventArgs e)
		{
			...
			if(!this._userSelectionStart.IsEmpty)
			{
				...
				this._userSelectionStart = SKPoint.Empty;
			}
		}
		...
		internal void Cursor_MouseMove(System.Windows.Forms.MouseEventArgs e, ref bool handled)
		{
			...
			if(this._userSelectionStart != SKPoint.Empty)
			{
				...
				if(this.IsUserEnabled)
				{
					SetCursorPixelPosition(new SKPoint(e.X, e.Y), true);
				}
				if(this.IsUserSelectionEnabled)
				{
					// Set selection
					SetSelectionPixelPosition(SKPoint.Empty, new SKPoint(e.X, e.Y), true);
				}
...
#region Cursor helper methods
		...
		private double PositionToCursorPosition(SKPoint position)
		{
			// Detect if cursor is horizontal or vertical
			bool	horizontal = true;
			if(this.GetAxis().AxisPosition == AxisPosition.Bottom || this.GetAxis().AxisPosition == AxisPosition.Top)
		...
		/// <returns>SKPoint.IsEmpty or relative coordinates in plotting area.</returns>
		private SKPoint GetPositionInPlotArea(SKPoint point, bool roundToBoundary)
		{
