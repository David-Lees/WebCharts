...
namespace WebCharts.Services.Models.General
{
    ...
    public partial class ChartGraphics
	{
			...
			)
		{
			...
			if( common.ProcessModePaint )
			{
				...
				((ChartGraphics)this).DrawLineRel(color, width, style, points[0].SKPoint, points[1].SKPoint);
			}
			...
			)
		{
            ...
            using (SKPath path = new SKPath())
            {
                ...
                SKRect pieTopRectangle = new SKRect();
                ...
                SKRect pieBottomRectangle = new SKRect();
		...
		internal void FillPieSlice( ChartArea area, DataPoint point, SolidBrush brush, Pen pen, SKPoint firstRectPoint, SKPoint firstPoint, SKPoint secondRectPoint, SKPoint secondPoint, SKPoint center, float startAngle, float sweepAngle, bool fill, int pointIndex )
		{
			// Common elements
			CommonElements common = area.Common;
            ...
            using (SKPath path = new SKPath())
            {
                ...
                // rectangle. This code will create SKRect from 
                // top left and bottom right points. 
                SKRect pieRectangle = new SKRect();
		...
		/// <param name="pointIndex">Data Point Index</param>
		internal void FillDoughnutSlice( ChartArea area, DataPoint point, SolidBrush brush, Pen pen, SKPoint firstRectPoint, SKPoint firstPoint, SKPoint secondRectPoint, SKPoint secondPoint, SKPoint threePoint, SKPoint fourPoint, float startAngle, float sweepAngle, bool fill, float doughnutRadius, int pointIndex )
		{
			// Common Elements
			CommonElements common = area.Common;
		...
		private void DrawSKPath( Pen pen, SKPath path )
		{
			...
			int yValueIndex)
		{
			...
			SKPath	splineSurfacePath = new SKPath();
			...
			bool clipInsideArea)
		{
			...
			SKPoint[] splinePathPoints = splineSurfacePath.PathPoints;
                        ...
                        case (MarkerStyle.Circle):
                            {
                                if ((operationType & DrawingOperationTypes.DrawElement) == DrawingOperationTypes.DrawElement)
                                {
                                        ...
                                        else
                                        {
                                            ...
                                            using (SKPath path = new SKPath())
                                            {
                                                ...
                                                using (PathGradientBrush shadowBrush = new PathGradientBrush(path))
                                                {
                                                    ...
                                                    shadowBrush.CenterPoint = new SKPoint(markerRotatedPosition.X, markerRotatedPosition.Y);
                                    ...
                                    {
                                        SKRect rectLightCenter = new SKRect(rect.Location, rect.Size);
                                        ...
                                        using (PathGradientBrush circleBrush = new PathGradientBrush(brushPath))
                                        {
                                            ...
                                            circleBrush.CenterPoint = centerPoint[0].SKPoint;
								...
								if(lightStyle != LightStyle.None)
								{
									...
									if(boundsRect.Height > 0 && boundsRect.Width > 0)
									{
											...
											!rightOppSideLinePoint.IsEmpty)
										{
											...
											SKPoint	middlePoint1 = ChartGraphics.GetLinesIntersection(
											...
											SKPoint	middlePoint2 = ChartGraphics.GetLinesIntersection(
												boundsRectMiddlePoint.X, boundsRectMiddlePoint.Y,
												centralLinePoint.X, centralLinePoint.Y,
			...
			out SKPoint rightSideLinePoint)
		{
			...
			int			rigthTopPoint = 0;
			SKPoint[]	ellipsePoints = ellipseFlattenPath.PathPoints;			
			...
			else
			{
				...
				else
				{
					...
					SKPoint[]	oppositeEllipsePoints = oppositeEllipseFlattenPath.PathPoints;			
    ...
    public class Point3D
	{
		...
		private	SKPoint	_coordXY = new SKPoint(0f, 0f);
		...
		public SKPoint SKPoint
		{
			...
			set
			{
                this._coordXY = new SKPoint(value.X, value.Y);
			}
		}
