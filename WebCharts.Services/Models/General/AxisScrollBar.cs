...
namespace WebCharts.Services.Models.General
{
    ...
    public class AxisScrollBar : IDisposable
	{
		...
		private SKPoint					_lastClickMousePosition = SKPoint.Empty;
...
#region Scroll bar painting methods
		...
		internal void Paint( ChartGraphics graph )
		{
			...
			SKRect	scrollBarRect = this.GetScrollBarRect();
			...
			SKSize	borderRelativeSize = new SKSize(borderWidth, borderWidth);
			borderRelativeSize = graph.GetRelativeSize(borderRelativeSize);
			SKRect scrollBarClientRect = new SKRect(scrollBarRect.Location, scrollBarRect.Size);
            ...
            foreach (ScrollBarButtonType buttonType in Enum.GetValues(typeof(ScrollBarButtonType)))
			{
				...
				SKRect buttonRect = this.GetScrollBarButtonRect(scrollBarClientRect, (ScrollBarButtonType)buttonType);
			...
			int borderWidth)
		{
			...
			SKRect	connectorRect = new SKRect(scrollBarRect.Location, scrollBarRect.Size);
			...
			ScrollBarButtonType buttonType)
		{
			...
			if(!pressedState)
			{
				...
				SKSize	pixelRelativeSize = new SKSize(1, 1);
				...
				graph.DrawLineRel(
					...
					new SKPoint(buttonRect.X, buttonRect.Bottom),
					new SKPoint(buttonRect.X, buttonRect.Top));
				graph.DrawLineRel(
					(singlePixelBorder) ? lighterColor : _buttonCurrentColor, 1, ChartDashStyle.Solid,
					new SKPoint(buttonRect.Left, buttonRect.Y),
					new SKPoint(buttonRect.Right, buttonRect.Y));
				...
				if(!singlePixelBorder)
				{
					...
					graph.DrawLineRel(
						...
						new SKPoint(buttonRect.X+pixelRelativeSize.Width, buttonRect.Bottom-pixelRelativeSize.Height),
						new SKPoint(buttonRect.X+pixelRelativeSize.Width, buttonRect.Top+pixelRelativeSize.Height));
					graph.DrawLineRel(
						lighterColor, 1, ChartDashStyle.Solid,
						new SKPoint(buttonRect.Left+pixelRelativeSize.Width, buttonRect.Y+pixelRelativeSize.Height),
						new SKPoint(buttonRect.Right-pixelRelativeSize.Width, buttonRect.Y+pixelRelativeSize.Height));
				}
			}
...
#region Mouse events handling for the Scroll Bar
		...
		internal void ScrollBar_MouseDown(object sender, System.Windows.Forms.MouseEventArgs e)
		{
			...
			if(e.Button == MouseButtons.Left && this.IsVisible)
			{
				...
				_lastClickMousePosition = new SKPoint(e.X, e.Y);
				...
				switch(buttonType)
				{
					...
					case(ScrollBarButtonType.ThumbTracker):
					{
							...
							(_lastClickMousePosition.X != x || _lastClickMousePosition.Y != y))
						{
							// Get scroll bar client rectangle
							SKRect	scrollBarRect = this.GetScrollBarRect();
							SKSize	borderRelativeSize = new SKSize(1, 1);
							...
							SKSize	buttonSize = new SKSize(scrollBarClientRect.Width, scrollBarClientRect.Height);
		...
		private void SetHotRegionElement( CommonElements common )
		{
			...
			foreach(object type in Enum.GetValues(typeof(ScrollBarButtonType)))
			{
				...
				// Get button rectangle
				SKRect buttonRect = this.GetScrollBarButtonRect(scrollBarClientRect, (ScrollBarButtonType)type);
...
#region Scroll bar helper methods
		...
		internal SKRect GetScrollBarButtonRect(SKRect scrollBarClientRect, ScrollBarButtonType buttonType)
		{
			// Initialize button rectangle
			SKRect	buttonRect = new SKRect(scrollBarClientRect.Location, scrollBarClientRect.Size);
		...
		/// </summary>
		/// <returns>Scroll bar position.</returns>
		internal SKRect GetScrollBarRect()
		{
			...
			//SKSize axisLineSize = new SKSize(axis.LineWidth, axis.LineWidth);
			...
			SKRect	areaPosition = axis.PlotAreaPosition.ToSKRect();
			if(!this.IsPositionedInside)
			{
				areaPosition = axis.ChartArea.Position.ToSKRect();
			...
			SKRect	barPosition = SKRect.Empty;
