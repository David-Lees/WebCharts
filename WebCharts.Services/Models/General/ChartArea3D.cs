...
namespace WebCharts.Services.Models.General
{
	...
	public partial class ChartArea
	{
		...
		internal void DrawArea3DScene(ChartGraphics graph, SKRect position)
		{
			...
			areaSceneWallWidth = graph.GetRelativeSize( new SKSize(this.Area3DStyle.WallWidth, this.Area3DStyle.WallWidth));
			...
			// Draw back wall
			SKRect	wallRect2D = new SKRect(position.Location, position.Size);
			...
			wallRect2D = new SKRect(position.Location, position.Size);
		...
		internal Point3D GetCenterOfProjection(COPCoordinates coord)
		{
			...
			SKSize	interval = new SKSize(0.5f, 0.5f);
