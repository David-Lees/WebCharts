    ...
    public partial class Axis
    {
        ...
        internal void PostFillLabels()
        {
            ...
            if (
                !LabelStyle.Enabled ||
                !enabled ||
                !string.IsNullOrEmpty(SubAxisName) ||
                axisType == AxisName.Y ||
            ...
            )
        {
            ...
            foreach (string seriesName in series)
            {
                ...
                if ((axisType == AxisName.X || axisType == AxisName.X2) && (margin != 0 || maxPointCount == 1 || !_autoMinimum) && !ser.IsXValueIndexed && ser.Points[0].AxisLabel.Length > 0 && ser.Points[^1].AxisLabel.Length > 0)
                {
                    allEmpty = false;
                }
